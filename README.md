...
## 有 PPPoE 账号，就是“独立宽带”吗？

**不能只凭 PPPoE 账号下结论。** 拨号方式、资费关系、线路资源和公网地址是不同维度：

| 想确认的事情 | 光猫配置能说明什么 | 还需要什么依据 |
|---|---|---|
| 技术上是否使用 PPPoE 拨号 | INTERNET WAN 的配置及运行中的 `pppd` 参数可用于确认拨号方式和账号；连接是否成功还需看运行状态 | 已建立的 PPP 会话或 WAN 在线状态 |
| 是否单独办理、单独收费 | 有自己的拨号账号，不代表资费独立；融合套餐也可以使用 PPPoE 账号 | 运营商业务订单、套餐详情、账单或客服确认 |
| 是否专线、独享物理线路或保证带宽 | WAN 名称、VLAN、NAT 状态和账号都不能证明这些属性 | 业务合同、接入方案及服务等级约定 |
| 是否有公网 IPv4、是否固定 | PPPoE 拨号本身不能证明公网或固定地址 | 检查实际 INTERNET WAN IPv4、与同一线路直连外网的出口 IPv4 对照，并核对地址分配约定 |

因此，“独立 PPPoE 拨号 WAN”在本文中**仅指使用该账号建立的逻辑上网连接**，不代表独立资费、专线或固定公网 IP。账号看起来像手机号或带地区后缀，也不能证明与某个手机套餐的绑定关系。

检查公网 IPv4 时，应排除代理、VPN 和多 WAN 分流的影响，并使用运行状态中的实际地址，而不是配置中的 `0.0.0.0` 占位值。若 WAN 地址为私网地址或 `100.64.0.0/10` 共享地址，说明光猫 WAN 未直接获得公网 IPv4；若 WAN 地址本身是公网地址且与出口地址一致，支持其直接获得该公网 IPv4 的判断，但不保证外部可入站访问。即使地址长时间不变，也不能据此确认是固定 IP。
