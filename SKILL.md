...
## 解读边界：PPPoE 拨号不等于独立资费或专线

用户问“这是独立宽带吗”时，先区分其含义，不要仅凭拨号账号回答“是”：

- **拨号方式**：INTERNET WAN 配置、`pppoe-` 接口名和运行中的 `pppd` 参数可相互佐证使用 PPPoE；进程存在不等于认证成功，在线状态应另行检查。
- **资费关系**：独立办理或手机融合套餐需要订单、套餐详情、账单或运营商确认。拨号账号、地区后缀、VLAN 和 NAT 状态均不能证明独立收费或手机套餐绑定关系。
- **线路资源**：逻辑 WAN 不等于专线、独享物理线路或保证带宽。相关属性须以业务合同、接入方案和服务等级约定为准。
- **公网地址**：PPPoE 不意味着公网 IPv4，更不意味着固定 IP。应读取运行状态中的实际 INTERNET WAN IPv4，而非配置占位地址；排除代理、VPN、多 WAN 分流后，与同一线路直连外网的出口 IPv4 对照。私网或 `100.64.0.0/10` 共享地址不是公网 IPv4；公网 WAN 地址与出口一致也不保证外部可入站访问或固定地址分配。

建议表述：“可以确认使用 PPPoE 账号拨号，但仅凭光猫配置无法判断是否单独收费、是否专线或是否分配固定公网 IP。”不要将“独立拨号 WAN”作为资费或独享资源结论。

### 公开提交与个人记录隔离

- 向 GitHub 提交时仅提交通用步骤、字段说明和虚构示例；不要复制完整用户记忆文件、配置备份、会话日志或真实账号密码。
- 用户要求“保存记录”不等于授权公开上传。若“提交这个”可能指个人凭据记录，先确认提交范围。
- 只暂存明确的文件，并检查暂存差异、PR 标题和正文是否包含敏感信息。不得上传真实凭据后再以新提交删除来代替提交前检查；旧提交和 PR 仍可能保留原值。
